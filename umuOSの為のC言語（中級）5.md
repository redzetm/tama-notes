...
この章では、プロセスの識別、`fork()` と `exec()` による起動、`exit()` と `_exit()` の違い、
`wait()` / `waitpid()` による終了回収、ユーザとグループ、セッションとプロセスグループ、
そしてデーモンの基本まで見てきました。
...
次の段階では、より高度なプロセス制御、スケジューリング、資源制限、シグナル連携、ジョブ制御の細部などを
理解すると、シェルやOSの設計がさらに立体的に見えてきます。
