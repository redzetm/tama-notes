...
ただし、現在のLinuxではsystemdなどのサービスマネージャがデーモン管理を行うため、
昔ながらの二重forkによるデーモン化を自前で書く場面は減っています。
...
シェルでパイプラインを作る場合、最初の子プロセスのpidをpgidにして、パイプライン内の他の子プロセスも
同じpgidへ入れる、という設計になります。
