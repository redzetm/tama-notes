...
- [systemctl(1) systemd サービス操作・状態確認（実用ノート）]({{ '/man_rocky9.7/systemctl.html' | relative_url }})（status/is-active、restart、enable、daemon-reload、ログ導線）
- [journalctl(1) systemd journal ログ調査（実用ノート）]({{ '/man_rocky9.7/journalctl.html' | relative_url }})（`-u/-b`、`-f`追跡、`--since`、`-p`重要度、`--list-boots`）
...
- [chmod(1) パーミッション変更（実用ノート）]({{ '/man_rocky9.7/chmod.html' | relative_url }})（8進数`644/755`、記号形式`u+x`、再帰`-R`注意、`a+X`、setgid/sticky）
