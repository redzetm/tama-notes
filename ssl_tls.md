...
## Rocky Linux / CentOS Stream 9 と KTLS（現実的な見方）
...
- Rocky9/CentOS Stream 9 では、カーネル側に KTLS 機能が入っていることは多い
...
「KTLSを使いたい」って言い出した瞬間に、最低限これを埋めると会話が前に進みます。
...
## 雑談：この話題で“技術雑談できる若手”を見分ける質問

面接というより、ふだんの雑談で相手の理解の軸が見えるやつ。

- 「TCP と TLS のハンドシェイク、目的の違いを一言で言うと？」
- 「証明書って、TLS のどのフェーズで何に使う？」
- 「SAN と CN の違い、今どっちを見る？」
- 「KTLS は“どこからどこまで”を速くする話？」
...
- 公式というより“確認用”
	- OpenSSL：`openssl s_client` の出力でプロトコル/暗号を観測
	- Linux：`/proc/net/tls_stat` で KTLS の統計観測

- 関連ノート
	- `setup-lets-encrypt.md`：証明書取得・更新・Webサーバ反映まわり
