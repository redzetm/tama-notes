...
　systemdタイマーを自動で設定してくれる。 　全部終わったら systemdctl list-timers で確認する。
...
出力
```text
メタデータの期限切れの最終確認: 0:41:59 前の 2026年01月13日 14時40分40秒 に実施しました。
依存関係が解決しました。
行うべきことはありません。
完了しました!
```
...
## ９　公開情報でもHTTPSを使わなければならない理由

「ログイン無しの公開ページだし、別にHTTPでもいいんじゃね？」となりがちだけど、今の時代は基本HTTPSが前提と思った方が良いです。
理由は「盗聴される」だけじゃなくて、**改ざんされる**のが一番怖い。

HTTPだと、通信途中でページの中身を差し替えられても利用者側は気づきにくいです。
例えば、広告を勝手に埋め込まれる、別サイトへ誘導リンクを混ぜられる、ダウンロードファイルを差し替えられる、みたいなことが起きうる。
公開情報でも「自身のサイトが踏み台にされる」と信用が終わります。

あと、公開情報でも意外と漏れるものがある。
アクセスしたページ、検索キーワード、リンクの踏み先、場合によってはリファラ等々。
内容が公開だからOKではなく、**利用者の行動履歴**が丸見えになるのは普通に嫌ですね！

実務的には次の理由も大きいです。

- ブラウザが「保護されていない通信」扱いにして、警告や機能制限が入る
- Cookieやログインや問い合わせフォームを後で足すとき、HTTPが混ざってると一気に地獄
- 外部サービス（OAuth、決済、API連携）でHTTPS必須が多い

ということで、公開情報であっても **最初からHTTPSにしておく** のが結局一番ラク。
Let's Encryptなら無料で出来るし、更新も自動化できるので、もうHTTPを残す理由があまりないです。
