...
このノートは、TCP/IP通信を「C言語で読む」ための入門となります。

まずは `tcpdump` や `Wireshark` で見える通信が、Cのプログラムからはどのような **バイト列** として見えるのかを
理解することになります。
...
- `if ((i + 1) % 16 == 0)` は、16バイト表示するごとに改行するための処理
- 最後の `if (len % 16 != 0)` は、データ数が16の倍数で終わらなかった場合にも最後に改行するための処理

`dump_bytes()` は、パケットの中身を16進数で横に並べて表示します。
ただし、全部を1行に出すと長くなりすぎて読みにくくなります。
そこで、よくある16進ダンプのように **16バイトごとに1行** にしています。

例えば `i` は0から始まります。
配列の添字は0始まりなので、1個目のバイトを表示しているとき `i` は0です。
...
「16個目を表示したら改行したい」ので、`i` そのものではなく `i + 1` を使います。
...
`%` は剰余、つまり割り算の余りを求める演算子です。
`(i + 1) % 16 == 0` は、「表示した個数が16で割り切れるか」を調べています。

```text
i = 0   -> i + 1 = 1   -> 1 % 16 = 1   -> 改行しない
i = 1   -> i + 1 = 2   -> 2 % 16 = 2   -> 改行しない
...
i = 15  -> i + 1 = 16  -> 16 % 16 = 0  -> 改行する
i = 31  -> i + 1 = 32  -> 32 % 16 = 0  -> 改行する
```

つまり、この部分は「16バイト読んだら行を変える」という意味です。
...
これは、データの長さが16の倍数ではない場合に、最後の行をきちんと改行して終えるためのものです。

今回のサンプルは12バイトです。
12は16の倍数ではないので、ループ中の `if ((i + 1) % 16 == 0)` は一度も成立しません。
そのままだと最後に改行されないため、ループの後で `len % 16 != 0` を見て改行しています。
...
もしデータがちょうど16バイトなら、16個目を表示した時点でループ内で改行されます。
その場合、さらに最後でもう一度改行すると空行が増えてしまうので、`len % 16 != 0` のときだけ改行しています。
