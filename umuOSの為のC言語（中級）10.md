...
そのため、研究上は「問題そのものは重要だが、現代 Linux では移行がかなり進んでいる」と押さえる方が実態に合います。
...
古い Linux では uptime ベースの説明がされることもありましたが、今の研究では「差分を取るための値」と考える方が分かりやすいです。
...
古い本では RFC 1305 との関連が前面に出ますが、研究上は「Linux が高機能な時計補正インタフェースを持っている」と理解すればまず十分です。
...
実際には libc 実装も関与するため、研究上は「タイマ満了時に関数コールバック風に処理できる高水準通知」と考える方が安全です。
